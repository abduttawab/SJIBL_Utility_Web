...
import { NescoPaymentService } from 'src/app/shared/NescoPayment.service';
    @Component({
      selector: 'app-NescoVerify.component',
      templateUrl: './NescoVerify.component.html',
      styles: []
    })
    export class NescoVerifyComponent implements OnInit {
      Districts = [];
      BranchList = [];
      SourceByList = [];
      SalesAgentList = [];
        ...
        this.commonService.getUtilityType().subscribe(
          (res:any) =>{
            ...
            this.UtilityTypes = Object.assign([], this.UtilityTypes).filter(
              item => item.companyShortCode.indexOf("NESCO") > -1
           );
              ...
              this.service.verify_Payment().subscribe(
                (res: any) => {
                  ...
                  if (res.isSuccessfull) {
                    ...
                    this.router.navigate(['/home/NescoFinalPayment'], { queryParams: { TransactionId: res.data } });
                    this.spinner.hide();

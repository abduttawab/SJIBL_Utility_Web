...
import { DpdcPaymentService } from 'src/app/shared/DpdcPayment.service';
    @Component({
      selector: 'app-DpdcVerify.component',
      templateUrl: './DpdcVerify.component.html',
      styles: []
    })
    export class DpdcVerifyComponent implements OnInit {
      Districts = [];
      BranchList = [];
      SourceByList = [];
      SalesAgentList = [];
        ...
        this.commonService.getUtilityType().subscribe(
          (res:any) =>{
            ...
            this.UtilityTypes = Object.assign([], this.UtilityTypes).filter(
              item => item.companyShortCode.indexOf("DPDC") > -1
           );
      ...
      resetWithDefault() {
        ...
        this.service.paymentFormModel.controls['Year'].setValue(new Date().getFullYear().toString());
        this.service.paymentFormModel.controls['Month'].setValue(('0'+new Date().getMonth()).toString().slice(-2));
              ...
              this.service.verify_Payment().subscribe(
                (res: any) => {
                  ...
                  if (res.isSuccessfull) {
                    ...
                    this.router.navigate(['/home/DpdcFinalPayment'], { queryParams: { TransactionId: res.data } });
                    this.spinner.hide();

...
@Component({
  selector: 'app-stage-md',
  templateUrl: './stage-md.component.html',
  styleUrls: ['./stage-md.component.css']
})
export class StageMdComponent implements OnInit {
 ...
 this.getData();
  ...
  onSearch() {
    this.service.search("EFFB907B-F7ED-42A5-8B3A-D248F570E928").subscribe(
      (res: any) => {
...
getData(){
  this.service.getApplicationsWithStatusId("EFFB907B-F7ED-42A5-8B3A-D248F570E928").subscribe(
    (res:any) =>{
      console.log(res);
      this.NewApplications=res.data;
    },
  ...
  changeStage(id: string) {
    if(confirm("Are you sure to sent this file to card operation?")) {
      this.service.changeStage(id,10).subscribe(
        (res: any) => {
  ...
  backStage(id: string) {
    if(confirm("Are you sure to sent this file to AMD Sir?")) {

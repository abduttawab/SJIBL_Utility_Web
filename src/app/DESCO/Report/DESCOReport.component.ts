...
@Component({
  selector: 'DESCOReport',
  templateUrl: './DESCOReport.component.html',
  styles: []
})
export class DESCOReport implements OnInit {
  AllData =  [];
...
generateReport(){
  ...
  if(this.service.reportModel.value.ReportType=='DESCO Postpaid Datewise Bill Collection Report'){

    this.desco_datewisereports();

    this.reportFileName = "SJIBL_DESCO_POSTPAID_DateWise_"+datestr;
  }
  if(this.service.reportModel.value.ReportType=='DESCO Postpaid Daily Bill Collection Details Report'){

    this.desco_detailsreports();

    this.reportFileName = "SJIBL_DESCO_POSTPAID_Daily_"+datestr;
  }
...
jspdf(filename){
  ...
  doc.text("Dhaka Electric Supply Company Limited", pageWidth / 2, 18, {align: 'center'});
  ...
  doc.addImage("assets/images/Logo_of_DESCO.png", "PNG", 170, 8, 25, 20);
...
DSS(doc){
  ...
  desco_datewisereports(){
    ...
    this.service.desco_datewisereports().subscribe(
      (res: any) => {
...
desco_detailsreports(){
  ...
  this.service.desco_detailsreports().subscribe(
    (res: any) => {

...
@Component({
  selector: 'app-BKASHFinalPayment.component',
  templateUrl: './BKASHFinalPayment.component.html',
  styles: []
})
export class BKASHFinalPaymentComponent implements OnInit {
  Districts = [];
  BranchList = [];
  SourceByList = [];
  SalesAgentList = [];
  ...
  getVerifiedTrans(traxId) {
    this.service.getverified(traxId).subscribe(
      (res: any) => {
        ...
        this.TransactionInfo = res.data.bkashTransactionInfo;
        ...
        this.service.paymentFormModel.controls['ShortCode'].setValue(res.data.customerCode);
       
        
        this.service.paymentFormModel.controls['Amount'].setValue(res.data.amount);
        this.service.paymentFormModel.controls['ConfirmAmount'].setValue(res.data.amount);

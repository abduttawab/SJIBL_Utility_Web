...
import { BKASHPaymentService } from 'src/app/shared/BKASHPayment.service';
    @Component({
      selector: 'app-BKASHVerify.component',
      templateUrl: './BKASHVerify.component.html',
      styles: []
    })
    export class BKASHVerifyComponent implements OnInit {
      Districts = [];
      BranchList = [];
      SourceByList = [];
      SalesAgentList = [];
      ...
      constructor(private commonService: CommonService,
        public service: BKASHPaymentService,
        ...
        this.commonService.getUtilityType().subscribe(
          (res:any) =>{
            ...
            this.UtilityTypes = Object.assign([], this.UtilityTypes).filter(
              item => item.companyShortCode.indexOf("BKASH") > -1
           );
  ...
  this.service.paymentFormModel.get('Amount').valueChanges.subscribe(() => {
    this.service.paymentFormModel.get('ConfirmAmount').updateValueAndValidity({ onlySelf: false, emitEvent: false });
  });

  this.service.paymentFormModel.get('ConfirmAmount').valueChanges.subscribe(() => {
    this.service.paymentFormModel.get('Amount').updateValueAndValidity({ onlySelf: false, emitEvent: false });
  });
      }
              ...
              this.service.verify_Payment().subscribe(
                (res: any) => {
                  ...
                  if (res.isSuccessfull) {
                    ...
                    this.router.navigate(['/home/BKASHFinalPayment'], { queryParams: { TransactionId: res.data } });
                    this.spinner.hide();
